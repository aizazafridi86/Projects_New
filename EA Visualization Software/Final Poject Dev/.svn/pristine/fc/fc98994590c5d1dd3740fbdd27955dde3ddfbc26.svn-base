...
namespace EA_Visualization_Software.PSO_Algorithm
{
    public class Particle
    {
        ...
        public Particle(string inputFunction,double lowerRange,double higherRange,int posClampVelocity,int negClampVelocity,int numberOfTimeSteps, List<string> uniqueGenesFound)
        {
        ...
        public void Update_LinearDecreasingVelocity(Particle gbest, double wt)
        {
            ...
            for(int i=0; i<this.numberOfGenes; i++)
            {
                ...
                double factor1 = this.pbest[i] - this.genes[i];
                factor1 = factor1 * c1 * r1;

                double factor2 = gbest.genes[i] - this.genes[i];
                factor2 = factor2 * c2 * r2;

                double add1 = factor1 + factor2;
                double add2 = this.velocity[i] * wt;

                this.velocity[i] = add1 + add2;

                //linear decreasing velocity update equation
        ...
        public void Update_Position()
        {
            ...
            for(int i=0; i<this.numberOfGenes ; i++)
            {
                newGeneValue = this.genes[i] + this.velocity[i];
       
                //Update genes values only if it is withing the function range
                if(newGeneValue < this.higherRange && newGeneValue >this.lowerRange)
                {
                    this.genes[i] = newGeneValue;
                }
                //if gene value is out of range,then it is not updated and velocity is clampred to 0
                else
                {
                    this.velocity[i] = 0;
                }
