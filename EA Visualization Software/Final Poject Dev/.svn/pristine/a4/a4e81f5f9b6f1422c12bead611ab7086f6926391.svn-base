    ...
    public class Recombination
    {
        ...
        public List<Offspring> onepoint_crossover(List<Offspring> offspring, int offspring_size, double crossover_rate, int number_genes,Random random)
        {
            ...
            for (int i=0; i<offspring_size; i=i+2)
            {
                ...
                //the second condition in if statement is to deal with the odd offspring population size
                if (this.generated_number <= crossover_rate && i+1<offspring_size)
                {
        ...
        //method for whole arithmetic recombination
        public List<Offspring> arithmetic_whole_recombination(List<Offspring> offspring, int offspring_size, double crossover_rate, double a, int number_genes,Random random)
        {
            double average;
            double genes_average;
            ...
            for(int i=0; i<offspring_size; i=i+2)
            {
                ...
                if (this.generated_number <= crossover_rate && i+1<offspring_size)
                {
                    ...
                    {
                        //calculating genes values using whole arithmetic recombination method
                        genes_average = (offspring[i].genes[j] + offspring[i + 1].genes[j]) / 2;
                        average = a * genes_average + (1 - a) * genes_average;

                        //copying the average calculated above to both offsprings genes
                        offspring[i].genes[j] = average;
                        offspring[i + 1].genes[j] = average;
                    }    
                }
            }
